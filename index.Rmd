...
Secondly, you have been provided by an outside source with some data about the villages that have been used in this project. However, it has unfortunately been passed along in long format with individual rows for the villages' population, sampling weights and whether or not they recieved support from an NGO.
...
Exercise 2a: Using `pivot_longer` reshape the wide plot data into a new long data set, save this output as `plot_data_long`. 
...
Exercise 2b: Using `pivot_wider`, transform the data into wide format such that you only have one row per village. Save this to a new objecct titled `village_data_wide`
